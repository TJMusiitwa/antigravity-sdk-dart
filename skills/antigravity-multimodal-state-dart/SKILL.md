...
2. **Session Continuation**: Configure `SessionContinuationMode` in `LocalAgentConfig`:
   ...
   - `SessionContinuationMode.createOrResume`: Resumes session if available; creates new session otherwise.
   - `SessionContinuationMode.createOnly`: Forces fresh session creation.
...
- **Media Buffers**: Construct `MediaContent` from binary bytes (`Uint8List`), file paths (`fromFile`), with automatic MIME type detection.
...
- [ ] State mutations use `StateStore` or `context.set()` safely.
