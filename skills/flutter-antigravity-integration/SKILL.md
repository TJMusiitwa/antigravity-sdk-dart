...
# Flutter Integration with Google Antigravity SDK
...
- **Turn Cancellation**: Bind user abort actions (e.g. Stop Button) directly to `response.cancel()` or `agent.conversation.cancel()` to cleanly terminate ongoing streaming trajectories without unmounting the agent.
...
- **Dual-Stream UI & Token Telemetry**: Separate `thoughts` (internal reasoning process) and `textStream` into distinct `StreamBuilder` widgets, and display live trajectory token counts from `UsageUpdate` events.
...
- [ ] User abort actions trigger `response.cancel()` or `agent.conversation.cancel()` cleanly during active generation.
- [ ] UI splits streams for internal reasoning (`thoughts`) and final text output (`textStream`).
