...
2. **Key & Service Tier Lookup**: Pass `GEMINI_API_KEY` via `LocalAgentConfig(apiKey: "...")` or ensure the `GEMINI_API_KEY` environment variable is set. Optionally specify `ServiceTier.priority` for prioritized inference.
...
- [`cancellation.dart`](file://example/getting_started/cancellation.dart): Session turn cancellation via `response.cancel()`.
...
- [ ] Turn cancellation via `response.cancel()` or `agent.conversation.cancel()` terminates active trajectory gracefully when invoked.
